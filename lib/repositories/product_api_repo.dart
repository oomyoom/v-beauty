...
class AllProductRepository {
  Future<List<ProductModal>> fetchProducts() async {
    final response =
        await http.get(Uri.parse('http://${ApiConstants.getdata}'));
  ...
  Future<List<ProductModal>> searchProductByName(String searchName) async {
    List<ProductModal> product = await fetchProducts();
    List<ProductModal> filteredProduct = product
        .where((product) =>
