...
router.post("/user", async (req, res) => {
    ...
    const query3 = `INSERT INTO app_user_details(UserID, Email,FirstName,LastName,Gender,Tel,Provider,Address,Zipcode,Birthday) VALUES(?,?,?,?,?,?,?,?,?,?)`;
    ...
    try {
        ...
        dbCon.query(query1,[hashPassword,Email,UserName],(err, results, fields) => {
            if (err) {
                ...
                const unspecifiedValue = "ไม่ระบุ";
                const numunspecifiedValue = "1111111";
                const dateunspcifiedValue = "2012-12-12";
                dbCon.query(query2,[userId],(err) => {
                    if (err) {
                        console.log("On UserGroupID table Error : ",err);
                        return res.status(400).send();
                    }
                    dbCon.query(query3, [userId, Email, unspecifiedValue, unspecifiedValue, unspecifiedValue, numunspecifiedValue, numunspecifiedValue, unspecifiedValue, numunspecifiedValue, dateunspcifiedValue], (err) => {
                        if (err) {
